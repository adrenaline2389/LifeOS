...
- 任务划分目录：`doc/tasks/`
- 模块进度文件：`doc/tasks/progress.md`

重点阅读：

- `doc/tasks/app-shell.md`
- `doc/tasks/onboarding.md`
- `doc/tasks/question-schema.md`
- `doc/tasks/local-data.md`
- `doc/tasks/manual-generation.md`
- `doc/tasks/manual-panel.md`
- `doc/tasks/subsystem-recommendation.md`
- `doc/tasks/export-reset.md`
- `doc/tasks/retro-ui-system.md`
...
严格按照 `doc/design.md` 和 `doc/tasks/` 中的模块职责实现。
...
- 跟踪 `doc/tasks/progress.md` 中的模块进度。
- 拆分子 Agent 任务。
- 审核子 Agent 输出。
- 集成模块。
- 运行最终验证。
...
如果并行实现导致接口不一致，以 `doc/design.md` 的数据模型和模块关系为准，由主 Agent 统一收敛。
...
2. 确认该模块测试通过。
3. 确认没有引入云端依赖。
4. 确认模块边界符合 `doc/design.md`。
5. 由主 Agent 更新 `doc/tasks/progress.md`。
