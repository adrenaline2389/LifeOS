...
- 题目总数固定为 9 题。
- 第 1-8 题为多选题。
- 每道多选题最少选择 1 个选项，不设置 3 个选项上限。
- 第 1-8 题支持填写“其他”文本回答，“其他”计入有效回答数量。
- 第 9 题为短文本题。
- 第 9 题可以跳过。
...
- 第 1-8 题强制最少选择 1 个，不设置 3 个选项上限。
- 第 1-8 题支持填写“其他”文本回答。
- “其他”文本回答计入有效回答数量。
- 第 9 题允许跳过。
- 用户完成 onboarding 后再持久化回答。
- 回答按 question id、selected option ids 和 custom text 结构化保存。

测试用例：

- 第 1-8 题选择 0 个选项时不能继续。
- 第 1-8 题允许选择超过 3 个选项。
- 第 1-8 题只填写“其他”文本时也能继续。
- 第 9 题可以跳过。
- 能产出合法回答记录。
...
2. 每道多选题都强制至少 1 个有效回答，不设置 3 个选项上限。
3. 第 9 题可以跳过。
4. app 能生成个人说明书控制面板。
5. 待验证观察包含来源引用。
6. app 最多推荐 2 个子系统。
